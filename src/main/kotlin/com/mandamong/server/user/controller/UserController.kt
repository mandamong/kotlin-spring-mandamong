...
class UserController(
    ...
    ): ResponseEntity<ApiResponse<LoginResponse>> = ApiResponse.created(service.create(request))

    @PatchMapping(ApiPath.User.UPDATE)
    fun update(
        @ModelAttribute request: UpdateUserRequest,
        @AuthenticationPrincipal loginUser: LoginUser,
    ): ResponseEntity<ApiResponse<String?>> = ApiResponse.ok(service.update(request, loginUser.userId))
    ...
    fun initializePassword(
        @RequestBody request: InitializePasswordRequest,
    ): ResponseEntity<ApiResponse<UpdateUserRequest>> = ApiResponse.ok(service.initializePassword(request.email))
}
